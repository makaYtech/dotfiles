...
# -----------
#
# doas or doas -e (replacement for doasedit) will be inserted before the command
#
# ------------------------------------------------------------------------------
# Authors
# -------
...
doas-command-line() {
...

  {
    ...
    if [[ -z "$EDITOR" ]]; then
      case "$BUFFER" in
        doas\ -e\ *) __doas-replace-buffer "doas -e" "" ;;
    ...
    if [[ "$realcmd" = (\$EDITOR|$editorcmd|${editorcmd:c}) \
      ...
      __doas-replace-buffer "$cmd" "doas -e"
      return
    fi

    # Check for editor commands in the typed command and replace accordingly
    case "$BUFFER" in
      $editorcmd\ *) __doas-replace-buffer "$editorcmd" "doas -e" ;;
      \$EDITOR\ *) __doas-replace-buffer '$EDITOR' "doas -e" ;;
      doas\ -e\ *) __doas-replace-buffer "doas -e" "$EDITOR" ;;
      doas\ *) __doas-replace-buffer "doas" "" ;;
      *) LBUFFER="doas $LBUFFER" ;;
    esac
  } always {
